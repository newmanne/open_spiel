...
class StraightforwardAgent(AbstractAgent):
    ...
    def step(self, time_step, is_evaluation=False):
        ...
        # Grab from encoding
        # cpi = clock_profit_index(self.num_players, self.num_actions)
        # profits = information_state_tensor[cpi:cpi + self.num_actions]
        ...
        try:
            print('Writing LP')
            problem.writeLP(f'straightforward_2.lp')
            ...
            for var_id in range(len(bundle_variables)):
                if value(bundle_variables[var_id]) > .99: # Rounding stupidness
                    print('only 1 feasible bundle')
                    action = var_id_to_action[var_id]
