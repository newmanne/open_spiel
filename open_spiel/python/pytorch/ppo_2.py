...
class Agent(nn.Module):
    ...
    def step(self, time_step, is_evaluation=False):
        if is_evaluation:
            with torch.no_grad():
                obs = torch.Tensor(time_step.observations['info_state'][0])
...
if __name__ == "__main__":
    ...
    obs = torch.zeros((args.num_steps, args.num_envs, info_state_size)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            _, reward, done, info = envs.step(action.cpu().numpy())
        ...
        # flatten the batch
        b_obs = obs.reshape((-1, info_state_size))
